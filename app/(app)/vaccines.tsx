...
// Se não estiver usando AsyncStorage, substitua pelo seu método de persistência ou Contexto
import AsyncStorage from "@react-native-async-storage/async-storage";

import { DropdownFilter, type FilterOption } from "../../src/components/DropdownFilter";
...
import type { JSONAgeGroup, UpdateVaccine } from "../../src/services/vaccine/vaccine.service";
...
export default function Vaccines() {
    ...
    const [vaccineSelectedId, setVaccineDataSelectedId] = useState<number | null>(null);
    ...
    function filterVaccine(option: string) {
        ...
        // Reseta o carrossel para o primeiro item ao filtrar
        flatListRef.current?.scrollToOffset({ offset: 0, animated: true });
    }

    // Avançar ou retroceder os cards no FlatList nativo
    function changeVaccineCarousel(direction: "left" | "right") {
        let nextIndex = direction === "left" ? currentIndex - 1 : currentIndex + 1;

        if (nextIndex >= 0 && nextIndex < useVaccines.length) {
    ...
    function updateVaccineDate(date: Form) {
        if (!vaccineSelectedId || !idChild) return;
    ...
    // Buscar o ID da criança salvo localmente de forma assíncrona no Mobile
    useEffect(() => {
    ...
    return (
        <View className="flex-1 w-full bg-white px-2">
            ...
            {/* Modal de Data de Aplicação (Nativo) */}
                ...
                <View className="flex-1 justify-center items-center bg-black/50 px-4">
                    <View className="w-full max-w-sm bg-lilas-bg p-6 rounded-xl items-center shadow-lg">
                        ...
                        <View className="flex-row justify-center gap-4 w-full">
                            <TouchableOpacity
                                onPress={() => { setModalOpen(false); reset(); }}
            ...
            <View className="bg-primary rounded-xl flex-1 p-2 relative">

                {isLoading && !isError && <LoadingBaby message="Procurando vacinas" />}
                {isError && (
                ...
                {!isLoading && !isError && (
                    <FlatList
                        ...
                        renderItem={({ item: age_group_vaccine }) => (
                            // Descontando as margens e paddings internos para alinhar na tela perfeitamente
                            <View style={{ width: SCREEN_WIDTH - 24 }} className="bg-lilas-medium p-4 rounded-xl flex-1">
                                <Text className="text-primary-text font-bold text-xl mb-4 text-center">
                                    {age_group_vaccine.age_group_name}
                                </Text>

                                {/* Scroll vertical interno para as vacinas daquela faixa etária */}
                                        ...
                                        {age_group_vaccine.vaccines.map((vaccine: any) => (
                                            ...
                                            >
                                                ...
                                                <View className="px-3 py-2 w-full flex-row bg-primary justify-between">
                                                    <Text className="w-[50%] text-white font-semibold text-xs">Vacina</Text>
                                                    <Text className="w-[25%] text-white font-semibold text-xs text-center">Status</Text>
                                                    <Text className="w-[25%] text-white font-semibold text-xs text-right">Data</Text>
                                                </View>

                                                {/* Dados da Vacina */}
                                                <View className="p-3 gap-2 flex-1">
                                                    <View className="flex-row justify-between w-full">
                                                        <Text className="w-[50%] text-primary-text font-bold text-sm" numberOfLines={1}>
                                                            {vaccine.vaccine}
                                                        </Text>
                                                        <Text className={`w-[25%] text-center text-xs font-semibold ${vaccine.application_status === 0 ? "text-red-500" : "text-primary-text"}`}>
                                                            {vaccine.application_status === 0 ? "Pendente" : "Aplicada"}
                                                        </Text>
                                                        <Text className={`w-[25%] text-right text-xs ${vaccine.application_status === 0 ? "text-red-500" : "text-primary-text"}`}>
                                                            {vaccine.application_date == null ? "Pendente" : RoutineDate.formatedDate(vaccine.application_date)}
                                                        </Text>
                                                    </View>

                                                    <Text className="text-xs text-primary">
                                                        <Text className="text-primary-darker font-bold">Doenças evitadas: </Text>
                                                        {vaccine.prevented_diseases}
                                                    </Text>

                                                    <Text className="text-xs text-primary italic">
                                                        <Text className="text-primary-darker font-bold not-italic">Obs: </Text>
                                                        {vaccine.observation ? vaccine.observation : "Sem observações"}
                                                    </Text>
                                                </View>
